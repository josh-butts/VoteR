...
long long binarySearch(const T& item, const TemplatedArray<T>& array, const Comparator<T>& comparator) {
	unsigned long high = array.getCapacity() - 1;
	unsigned long low = 0;
	unsigned long mid;
	while (high >= low)
	{
		mid = (high + low) / 2;
		if (comparator.compare(item, array[mid]) == -1) // it's less than mid
		{
			high = mid - 1;
		}
		else if (comparator.compare(item, array[mid]) == 1) // it's greater than mid
		{
			low = mid + 1;
		}
		else // it's found
